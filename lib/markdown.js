/*
  From https://github.com/developit/snarkdown
*/
...
export default function markdown(md, prevLinks) {
  let tokenizer = /((?:^|\n+)(?:\n---+|\* \*(?: \*)+)\n)|(?:^``` *(\w*)\n([\s\S]*?)\n```$)|((?:(?:^|\n+)(?:\t|  {2,}).+)+\n*)|((?:(?:^|\n)([>*+-]|\d+\.)\s+.*)+)|(?:!\[([^\]]*?)\]\(([^)]+?)\))|(\[)|(\](?:\(([^)]+?)\))?)|(?:(?:^|\n+)([^\s].*)\n(-{3,}|={3,})(?:\n+|$))|(?:(?:^|\n+)(#{1,6})\s*(.+)(?:\n+|$))|(?:`([^`].*?)`)|(  \n\n*|\n{2,}|__|\*\*|[_*]|~~)/gm,
  context = [],
  out = '',
  links = prevLinks || {},
  ...
  while ( (token=tokenizer.exec(md)) ) {
    ...
    // > Quotes, -* lists:
      ...
      inner = markdown(outdent(token[5].replace(/^\s*[>*+.-]/gm, '')));
      if (t=='>') t = 'blockquote';
      else {
        t = t.match(/\./) ? 'ol' : 'ul';
        inner = inner.replace(/^(.*)(\n|$)/gm, '<li>$1</li>');
    ...
    // Headings:
      ...
      chunk = '<'+t+'>' + markdown(token[12] || token[15], links) + '</'+t+'>';
    }
    // `code`:
    else if (token[16]) {
